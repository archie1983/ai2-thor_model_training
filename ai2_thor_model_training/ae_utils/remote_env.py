...
import socket, json, cv2, logging, threading, elements, random, traceback, pbd
import numpy as np
	...
	def handle_client(self, conn, addr):
		...
		try:
			...
			if init_command.get("command") == "INIT":
				hab_id = init_command.get("hab_id", "1")
				self.hab_set = init_command.get("hab_set", "train")
				...
				response = {"status": "READY",
							...
							"unreachable_postions": self.unreachable_postions,
