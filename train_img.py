...
def load_pretrained(path):
    assert os.path.isfile(path), f'Could not find DiT pretrained model at {path}'
...
def main(args):
    ...
    train_steps = 0
    if args.checkpoint_path != "null" and os.path.exists(args.checkpoint_path):
    ...
    if args.pretrained_path != "null" and os.path.exists(args.checkpoint_path):
        pretrained_model = load_pretrained(args.pretrained_path)
        state_dict = pretrained_model["ema"] if "ema" in pretrained_model else pretrained_model
        # Remove patch embedding and positional embedding
        filtered_state_dict = {k: v for k, v in state_dict.items()
                               if not k.startswith("patch_embed") and "pos_embed" not in k}
        model.module.load_state_dict(filtered_state_dict, strict=False)
        ema.load_state_dict(filtered_state_dict, strict=False)
        opt.load_state_dict(pretrained_model["opt"])
        logger.info(f"Loaded pretrained model from {args.checkpoint_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint-path", type=str, default="null")
    parser.add_argument("--pretrained-path", type=str, default="null")
